...
public class AsyncMySQLManager {
    ...
    public AsyncMySQLManager(String host, int port, String database, String username, String password) {
        this.host = host;
        this.port = port;
    ...
    public void connect() {
        try {
            ...
            try (Connection conn = java.sql.DriverManager.getConnection(
                ...
                if (plugin.isDebug()) plugin.getLogger().info("Database '" + database + "' ensured.");
            }
            ...
            if (plugin.isDebug()) plugin.getLogger().info("Connected to MySQL.");
        } catch (Exception e) {
    ...
    public void disconnect() {
        if (dataSource != null && !dataSource.isClosed()) {
            dataSource.close();
            executor.shutdown();
            if (plugin.isDebug()) plugin.getLogger().info("MySQL pool closed.");
        }
    }
    ...
    public void createTable(String table, String sqlStatement) {
        executeAsync(() -> {
            try (Connection connection = getConnection();
                ...
                ps.executeUpdate();
                 if (plugin.isDebug()) plugin.getLogger().info("Table '%table%' ensured.".replace("%table%", table));
            } catch (SQLException e) {
                 if (plugin.isDebug()) plugin.getLogger().severe("Failed to create table '%table%'.".replace("%table%", table));
                e.printStackTrace();
            }
        });
    }
